...
@Component({
  selector: 'app-extactec',
  templateUrl: './extactec.component.html',
  styles: [
  ]
})
export class EXTACTECComponent implements OnInit {
    ...
    this.solicitud=this.fb.group(      
      {
        datos: this.fb.group({
          ...
          tipoSolicitud:['EXTACTEC',[Validators.required]],
  ...
  guardar(){
    ...
    LimpiarFormulario(){
      this.solicitud.reset();
      this.solicitud.get('datos')?.get('tipoSolicitud')?.setValue('EXTACTEC');
    }
    //Imprimir
    public downloadPDF() {
      // Extraemos e
