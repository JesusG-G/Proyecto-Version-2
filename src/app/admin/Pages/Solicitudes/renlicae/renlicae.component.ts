...
@Component({
  selector: 'app-renlicae',
  templateUrl: './renlicae.component.html',
  styles: [
  ]
})
export class RENLICAEComponent implements OnInit {
    ...
    this.solicitud=this.fb.group(      
      {
        datos: this.fb.group({
          ...
          tipoSolicitud:['RENLICAE',[Validators.required]],
  ...
  guardar(){
    ...
    LimpiarFormulario(){
      this.solicitud.reset();
      this.solicitud.get('datos')?.get('tipoSolicitud')?.setValue('RENLICAE');
    }
    //Imprimir
    public downloadPDF() {
      // Extraemos e
